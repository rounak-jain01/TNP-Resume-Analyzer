...
def upload_jd(
    ...
    file_path, file_type = save_temp_file(file)
    ...
    try:
        ...
        jd = create_jd(
            db,
            current_user.id,
            source="faculty_upload"
        )

        jd = parse_and_save_jd(
            db,
            jd,
            raw_text
        )
...
def upload_batch(
    ...
    if len(files) == 0:
        raise HTTPException(
            status_code=400,
            detail="At least 1 resume is required"
        )

    if len(files) > MAX_BATCH_SIZE:
        raise HTTPException(
            status_code=400,
            detail=f"Maximum {MAX_BATCH_SIZE} resumes allowed per batch"
        )
...
def get_batch_insight_summary(
    ...
    if insights is None:
        raise HTTPException(
            status_code=404,
            detail="No analysis results found for this batch"
        )
...
def add_resumes_to_batch(
    ...
    if batch.total_resumes + len(files) > MAX_BATCH_SIZE:
        raise HTTPException(
            status_code=400,
            detail=(
                f"Batch would exceed {MAX_BATCH_SIZE} resumes "
                f"(currently {batch.total_resumes})"
            ),
        )

    jd = (
        db.query(JD)
        .filter(JD.id == batch.jd_id)
        .first()
    )
